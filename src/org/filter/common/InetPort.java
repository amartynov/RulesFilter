...
public class InetPort {
	...
	static {
		portList.put("ftp-data", 20);
		portList.put("ftp", 21);
		portList.put("telnet", 23);
		portList.put("smtp", 25);
		portList.put("http", 80);
		portList.put("imap2", 143);
		portList.put("domain", 53);
	}
	
	public static final int maxPort = 0xffff;
	...
	public InetPort(Integer port) {
		this.port = port;
	}
	
	public InetPort(Integer lPort, Integer rPort) {
		this.lPort = lPort;
		this.rPort = rPort;
	}
	
	public InetPort(String port) {
		this.setPort(port);
	}
	...
	public void setPort(String sPort) {
		this.port = portList.get(sPort);
	}
	
	public void setlPort(String sPort) {
		this.lPort = portList.get(sPort);
	}
	
	public void setrPort(String sPort) {
	...
	/**
	 * 
	 * @param list1 first ArrayList of InetPort
	 * @param list2 second ArrayList of InetPort
	 * @return ArrayList of intersection list1 and list2
	 */
  public static ArrayList<InetPort> intersection(ArrayList<InetPort> list1, ArrayList<InetPort> list2) {
    ArrayList<InetPort> res = new ArrayList<InetPort>();
    for(InetPort p1 : list1){
      for(InetPort p2 : list2){
        Integer port1 = p1.getPort();
        Integer port2 = p2.getPort();
        if(port1 != null && port2 != null) {
          //if port1 and port2 are numbers
          if(port1.intValue() == port2.intValue()){
            res.add(new InetPort(new Integer(p1.getPort())));
          }
        } else {
          if(port1 == null && port2 != null){
            //if port1 is interval and port2 is number
            if(port2.intValue() > p1.getlPort().intValue() && port2.intValue() < p1.getrPort().intValue()) {
              res.add(new InetPort(new Integer(p2.getPort())));
            }
            
          } else if(port1 != null && port2 == null){
            if(port1.intValue() > p2.getlPort().intValue() && port1.intValue() < p2.getrPort().intValue()) {
              res.add(new InetPort(new Integer(p1.getPort())));
            }
          } else {
            //if port1 and port2 are intervals
            Integer lport1 = p1.getlPort();
            Integer rport1 = p1.getrPort();
            Integer lport2 = p2.getlPort();
            Integer rport2 = p2.getrPort();
            Integer lp = null;
            Integer rp = null;
            
            //TODO: test
            if(lport1 <= lport2 && lport2 <= rport1){
              lp = new Integer(lport2);
              rp = rport1 >= rport2 ? new Integer(rport2) : new Integer(rport1);
            } else if(lport2 < lport1 && lport1 < rport2) {
              lp = new Integer(lport1);
              rp = rport1 > rport2 ? new Integer(rport2) : new Integer(rport1);
            }
            
            if(lp != null && rp != null){
              if(lp.intValue() == rp.intValue()){
                res.add(new InetPort(lp));
              } else {
                res.add(new InetPort(lp, rp));
              }
            }
          }
        }
